...
function upgradeNetwork() {
  if [[ "$IMAGETAG" == *"1.4"* ]] || [[ $IMAGETAG == "latest" ]]; then
    docker inspect -f '{{.Config.Volumes}}' orderer.superapp.epm.com | grep -q '/var/hyperledger/production/orderer'
    if [ $? -ne 0 ]; then
    ...
    docker-compose $COMPOSE_FILES stop orderer.superapp.epm.com
    docker cp -a orderer.superapp.epm.com:/var/hyperledger/production/orderer $LEDGERS_BACKUP/orderer.superapp.epm.com
    docker-compose $COMPOSE_FILES up -d --no-deps orderer.superapp.epm.com

    for PEER in peer0.org1.superapp.epm.com peer1.org1.superapp.epm.com peer0.org2.superapp.epm.com peer1.org2.superapp.epm.com; do
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    # remove the docker-compose yaml file that was customized to the superapp.epm
...
function replacePrivateKey() {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/org1.superapp.epm.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA1_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
  cd crypto-config/peerOrganizations/org2.superapp.epm.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
...
# (``peer0.org1.superapp.epm.com`` & ``peer0.org2.superapp.epm.com``).  Secondly, we point to
